...
layer-performance-build.mjs                Startup cost inside the layer: one cached Intl.DateTimeFormat per
                                           option set instead of one per call (was seconds of CPU per refresh).
preload-build.mjs                          Resource hints for the hosted/local builds: modulepreload for the
                                           add-on modules and their static imports at the end of <head>, and
                                           fetch preloads for the downtown set (manifest, terrain, roads,
                                           critical tiles, event baseline) after the bundle.
...
Both hosted and local builds extract the city's existing gzip/base64 payload from the HTML into `city-assets/*.bin`. The entry starts loading scripts immediately and the game's existing parallel fetches load the binary geometry relative to that build's directory. `preload-build.mjs` announces the same URLs ahead of time: the add-on modules (and the imports multiplayer pulls in) as `modulepreload` at the end of `<head>`, and the downtown set — city manifest, terrain, roads, the critical tiles, the survey points and `data/tech-week-first.json` — as `<link rel="preload" as="fetch" crossorigin>` after the bundle, so those downloads overlap with script evaluation and the React mount instead of waiting for them. The city files sit after the bundle on purpose: in `<head>` they shared bandwidth with the document itself on a plain HTTP/1.1 server and delayed it by their own transfer time. Background tiles, the north shore and the full event snapshot are not hinted; they stay behind the game's throttled queues. `events-sync.js` also reads the saved baseline as soon as it is evaluated, so the layer's first read after "downtown ready" finds it in memory. Three integer index arrays use lossless delta/varint encoding before gzip; floating-point positions and the other resources retain their original compressed bytes. Every decoded resource must match the original byte for byte. Serve or publish the entire output directory, including `city-assets/`; copying `index.html` alone is insufficient. The single-file artifact build retains its existing self-contained format. `npm test` rebuilds hosted/local outputs and verifies byte equivalence for all 18 resources, request concurrency, retry behavior, and entry size.

...
import { splitCityAssets, writeCityAssets } from './city-assets-build.mjs';
...
const cityAssets = splitCityAssets(original);
await rm(url('./dist-local/'), {recursive:true, force:true});
await mkdir(url('./dist-local/'), {recursive:true});
await writeFile(url('./dist-local/index.html'), renderGame(cityAssets.html));

// Turns the cached public data (manhattan/fetch-data.mjs) into the city
// resources the game engine already knows how to draw — the same buffers,
// attribute layouts and manifest shape as the San Francisco world:
//
...
//   city-manifest.json
//
// Output: manhattan/city/*.gz (canonical gzip, committed) + manifest.json.
//
//   node manhattan/generate-city.mjs
//
...
// illustrative palette inferred from construction year and height, exactly as
// the SF world infers them — nothing here is a surveyed facade colour.
import { readFile, writeFile, mkdir, rm } from 'node:fs/promises';
import { gzipSync } from 'node:zlib';
import { createHash } from 'node:crypto';
import { BOX, ORIGIN, METERS_LAT, METERS_LON, project, TERRAIN_BOUNDS, SOURCES, LANDMARKS } from './config.mjs';
import { triangulate } from './earcut.mjs';

const cache = new URL('./.cache/', import.meta.url);
const out = new URL('./city/', import.meta.url);
const FT = 0.3048;
...
const WATER_HEIGHT = -2.5;           // terrain under water sits below the water plane (y = 0.7)
const LAND_MIN_HEIGHT = 1.2;         // bulkheads never dip under the water plane
const NJ_LIMIT_LON = -74.012;        // west of the state line only the DEM tells land from river
const NJ_LAND_HEIGHT = 0.6;
...
if (dem.width !== nx || dem.height !== nz) throw new Error(`Elevation raster is ${dem.width}×${dem.height}, terrain grid is ${nx}×${nz}; re-run fetch-data.mjs`);
console.log(`inputs: ${footprints.length} footprints, ${centerlines.length} street segments, ${boroughs.length} boroughs, ${nx}×${nz} elevation`);

// Borough polygons (lon/lat) → land test with ring bounding boxes.
const landPolygons = [];
for (const borough of boroughs) for (const polygon of borough.the_geom.coordinates) {
  const rings = polygon.map(ring => ({ ring, box: bbox(ring) }));
  landPolygons.push(rings);
}
function isCityLand(lon, lat) {
...
// DEM pixel centres sit half a pixel inside the box; sample bilinearly at the
// exact grid node so the surface, the land test and the buildings agree.
function demAt(lon, lat) {
  const fx = (lon - BOX.west) / (BOX.east - BOX.west) * dem.width - 0.5, fy = (BOX.north - lat) / (BOX.north - BOX.south) * dem.height - 0.5;
  const x0 = Math.max(0, Math.min(dem.width - 2, Math.floor(fx))), y0 = Math.max(0, Math.min(dem.height - 2, Math.floor(fy)));
  const tx = Math.max(0, Math.min(1, fx - x0)), ty = Math.max(0, Math.min(1, fy - y0));
  const v = (x, y) => { const s = dem.data[y * dem.width + x]; return s < -1000 ? 0 : s; };
...
for (let j = 0; j < nz; j += 1) for (let i = 0; i < nx; i += 1) {
  ...
  const isLand = isCityLand(lon, lat) || (lon < NJ_LIMIT_LON && elevation >= NJ_LAND_HEIGHT);
...
for (const row of footprints) {
  ...
  let height = heightFeet > 0 ? heightFeet * FT : 0;
  const year = Number(row.construction_year) || 0;
  const seed = String(row.bin || hash32(JSON.stringify(row.the_geom.coordinates[0][0][0])));
  for (const polygon of row.the_geom.coordinates) {
    ...
    if (landmark) landmarkHits.set(landmark.id, { bin: row.bin, heightFeet, year });
    if (!(height >= 2.5)) height = landmark ? landmark.heightMeters : 4.5;   // unsurveyed: a low block, never a guess at a tower
...
console.log('landmarks:', [...landmarkHits].map(([id, hit]) => `${id}=${hit.bin}/${Math.round(hit.heightFeet * FT)}m`).join(' '));
...
let roadVertices = 0, roadFeatures = 0;
for (const row of centerlines) {
  if (!ROAD_TYPES.has(String(row.rw_type))) continue;
  const widthFeet = Number(row.streetwidth);
  let width = widthFeet > 0 ? widthFeet * FT : 9;
  if (row.rw_type === '6') width = Math.min(width, 6);
  width = Math.max(5, Math.min(30, width));
  const tone = row.rw_type === '2' ? 235 : width >= 22 ? 190 : 135;
  let drawn = false;
  for (const line of row.the_geom.coordinates) {
    const points = line.map(([lon, lat]) => { const p = project(lon, lat); return [p.x, p.z]; });
    for (let i = 0; i + 1 < points.length; i += 1) {
      ...
      roadVertices += 4; drawn = true;
    }
  }
  if (drawn) roadFeatures += 1;
}
...
const manifest = {
  ...
  buildings: { sourceFeatures: footprints.length, renderedBuildings: rendered, vertices: vertexCount, indices: B.index.length,
    ...
    setbacks: 'Towers above 90 m are stepped back on a seeded rule; the named landmarks follow their published massing. Footprints and roof heights stay surveyed.' },
  ...
  sources: [
    { ...SOURCES.footprints, fields: 'the_geom, height_roof (ft), ground_elevation, construction_year' },
    { ...SOURCES.centerlines, fields: 'the_geom, rw_type, streetwidth (ft)' },
    { ...SOURCES.boroughs, use: 'land mask on the New York side' },
    { ...SOURCES.elevation, use: `terrain grid ${nx}×${nz} at ${TERRAIN_BOUNDS ? Math.round(dx) : ''} m; land west of the state line where the bare-earth height is above ${NJ_LAND_HEIGHT} m` },
  ],
  box: BOX,
};
...
console.log(`wrote ${Object.keys(files).length} resources, ${(total / 1048576).toFixed(1)} MB compressed → manhattan/city/`);
